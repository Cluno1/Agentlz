...
**目标**
- 测试 Check Agent 的行为，包括完美匹配、部分匹配、不匹配等场景。
- 验证 Check Agent 的逻辑正确性和评分机制。

**运行命令**
- 在项目根目录：
  - `python -m test.check.test_check`

**输出**
- 测试日志：
  - 包括每个测试用例的目标、事实、判断结果、评分和理由。
...
**常见问题**
- 测试未通过：检查 `Check Agent` 的实现逻辑。
- 环境配置错误：确保依赖已安装并正确配置。

**关联文件**
- Check Agent：`agentlz/agents/check/check_agent_1.py`