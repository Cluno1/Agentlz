"""Agent‑Document 关联仓储（MySQL）

职责
- 管理 agent 与 document 的关联关系（唯一键 `agent_id + document_id`）
- 所有 SQL 使用参数化形式（sqlalchemy.text + 绑定参数），避免 SQL 注入

表结构对齐（参见 `docs/deploy/sql/init_mysql.sql` 的 `agent_document` 表）
- 主键：`id` bigint(20)
- 唯一约束：`uk_agent_document (agent_id, document_id)`
- 索引：`idx_agent_document_agent`、`idx_agent_document_document`

使用约定
- 查询/删除支持主键或唯一对（pair）两种方式
- 新增时避免重复：可在服务层先查 `get_agent_document_by_pair`
"""
...
def get_agent_document_by_pair(*, agent_id: int, document_id: str, table_name: str) -> Optional[Dict[str, Any]]:
    """按唯一键 `(agent_id, document_id)` 查询关联记录。"""
    sql = text(
        f"""
        SELECT id, agent_id, document_id, created_at
...
def delete_agent_document_by_pair(*, agent_id: int, document_id: str, table_name: str) -> bool:
    """按唯一键 `(agent_id, document_id)` 删除关联记录。"""
    sql = text(f"DELETE FROM `{table_name}` WHERE agent_id = :agent_id AND document_id = :document_id")
    engine = get_mysql_engine()
    with engine.begin() as conn:
        result = conn.execute(sql, {"agent_id": agent_id, "document_id": document_id})
        return result.rowcount > 0