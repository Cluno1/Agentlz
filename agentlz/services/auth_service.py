...
from agentlz.schemas.user import UserCreate
from agentlz.services import user_service

def login_service(*, tenant_id: str, username: str, password: str) -> str:
    s = get_settings()
    table = getattr(s, "user_table_name", "users")
    row = repo.get_user_by_username(username=username, tenant_id=tenant_id, table_name=table)
    if not row or int(row.get("disabled", 0)) == 1:
        raise ValueError("invalid_credentials")
    if (row.get("password_hash") or "") != password:
        raise ValueError("invalid_credentials")
