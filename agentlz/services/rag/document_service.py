...
from agentlz.services.rag.chunk_embeddings_service import (
    ...
    search_similar_chunks_service,
    chunk_content_by_strategy,
)
...
def _check_document_access_permission(
    *, document: Dict[str, Any], current_user_id: int, tenant_id: str
) -> bool:
    """检查用户是否有权限查看文档。
    ...
    perm_record = perm_repo.get_perm_by_user_doc(
        user_id=current_user_id, doc_id=document.get("id"), table_name=perm_table
    )
    if perm_record:
    ...
    user_info = user_repo.get_user_by_id(
        user_id=current_user_id, tenant_id=tenant_id, table_name="users"
    )
    if (
        user_info
        and user_info.get("role") == "admin"
        and user_info.get("tenant_id") == tenant_id
    ):
        return True
...
def get_document_service(
    *, doc_id: str, tenant_id: str, claims: Optional[Dict[str, Any]] = None
    ...
    if not _check_document_access_permission(
        document=row, current_user_id=current_user_id, tenant_id=tenant_id
    ):
...
def get_download_payload_service(
    *, doc_id: str, tenant_id: str, claims: Optional[Dict[str, Any]] = None
    ...
    if not row:
        ...
        if (
            any_row
            and current_user_id_tmp is not None
            and any_row.get("uploaded_by_user_id") == current_user_id_tmp
        ):
            row = any_row
        else:
            return None
...
def create_document_service(
    *, payload: DocumentUpload, tenant_id: str, claims: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
...
def publish_document_chunk_tasks_service(
    *,
    ...
    """为指定文档按策略发布分块解析任务到 RabbitMQ

    权限：
        - 复用 get_document_service 内部的鉴权逻辑（_check_document_access_permission）

    输入：
        - doc_id: 文档ID
        - strategies: 需要发布的分块策略列表（整数）
        - tenant_id: 当前租户ID
        - claims: 用户认证信息

    行为：
        - 校验策略列表有效性（去重、过滤非法值）
        - 获取文档信息（含 save_https、type）
        - 按策略逐条发布到队列 doc_parse_tasks，消息包含：doc_id、tenant_id、save_https、document_type、strategy
        - 失败时重试并记录错误

    返回：
        - {doc_id, tenant_id, published_count, published_strategies}
    """
    _ensure_authenticated(claims)
    # 权限校验与文档存在性检查
    row = get_document_service(doc_id=doc_id, tenant_id=tenant_id, claims=claims)
    if not row:
        raise HTTPException(status_code=404, detail="文档不存在")

    # 读取文档必要字段
    save_https = row.get("save_https")
    ...
    for s in strategies or []:
        ...
        cleaned.append(v)
    if not cleaned:
        raise HTTPException(status_code=400, detail="strategy 列表为空或无有效值")

    published = 0
    last_err: Optional[Exception] = None
    for strat in cleaned:
        msg = {
            "doc_id": doc_id,
            "save_https": save_https,
        ...
        try:
            publish_to_rabbitmq("doc_parse_tasks", msg, durable=True)
            published += 1
            logger.info(f"发布分块解析任务成功: {msg}")
        except Exception as e:
            logger.error(f"发布分块解析任务失败: {e}")
            last_err = e
            # 重试一次
            try:
                from agentlz.core.external_services import close_all_connections
                ...
                publish_to_rabbitmq("doc_parse_tasks", msg, durable=True)
                published += 1
                logger.info(f"重试发布分块解析任务成功: {msg}")
                last_err = None
            except Exception as retry_e:
                logger.error(f"重试发布分块解析任务失败: {retry_e}")
                last_err = retry_e
                # 不中断其他策略的发布，继续下一个
                continue
    ...
    if last_err is not None and published == 0:
        # 若全部失败，返回 500
        raise HTTPException(
            status_code=500, detail=f"RabbitMQ 发布任务失败: {str(last_err)}"
        )
    return result


def process_document_from_cos_https(
    save_https: str, document_type: str, doc_id: str, tenant_id: str, strategy: int = 0
) -> str:
    ...
    try:
            ...
            try:
                ...
                if not soffice:
                    ...
                    try:
                        ...
                        content = (
                            textract.process(ppt_path).decode("utf-8", "ignore").strip()
                        )
                        if content:
                            return content
                    except Exception:
                ...
                try:
                    ...
                    res = MarkItDown().convert_local(converted, file_extension=".pptx")
            ...
            try:
                ...
                if not soffice:
                    ...
                    try:
                        ...
                        content = (
                            textract.process(doc_path).decode("utf-8", "ignore").strip()
                        )
                        if content:
                            return content
                    except Exception:
                ...
                try:
                    ...
                    res = MarkItDown().convert_local(converted, file_extension=".docx")
                    return res.text_content
                finally:
                    shutil.rmtree(outdir, ignore_errors=True)
            finally:
        ...
        for chunk in chunks:
            index += 1
            logger.info(
                f"文档 {doc_id} 切割为Markdown块，长度: {len(chunk)} 字符，策略: {strategy}"
            )
            create_chunk_embedding_service(
                tenant_id=tenant_id,
